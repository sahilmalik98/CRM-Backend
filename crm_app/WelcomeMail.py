...
def send_welcome_mail(contact_name,form_data, leadid):
    ...
    smtp_port = 587
    smtp_user = 'it@rssscorp.com'
    smtp_password = 'Shivshakti1!'
    from_email = 'it@rssscorp.com'
    try:
    ...
    if mail_template.email_attachments.exists():
        for attachment in mail_template.email_attachments.all():
            try:
                ...
                file_name = attachment.name.name
...
def send_meeting_email(form_data, idd):
    ...
    try:
        MeetingDetails = Meeting.objects.get(id=idd)
    ...
    #logger.info(f"Email: {form_data['email']}")
    # Prepare the email notification
    notification_email = LeadDetails.email  # Replace with your recipient's email
    ...
    try:
        ...
        # Save email log to the database (assuming you have EmailSpecificFields model)
        try:
            email_log = EmailSpecificFields(
                sender='it@rssscorp.com',
